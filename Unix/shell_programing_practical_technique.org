...
* ファイルの取扱いとシステムの操作
** バックアップ/同期
コマンドの打ち間違いで，ディレクトリを消してしまうことがある．
この程度のミスは，自転車の練習でコケるのと同じ．
きちんとバックアップさえとっておけば，むしろ自転車でコケるより安全．
*** ディレクトリをバックアップする
**** 1つのファイルに圧縮して保存する
sudo tar zcvf /dir/to/backupfile.tar.gz ~/
- z: gzip
- c: compress
- v: verbose
- f: to file
...
#+RESULTS:
: -rw-r--r--  1 ahayashi  staff  103475 Jan  2 15:41 bk.tar.gz

**** より高い圧縮率での圧縮を試みる
j は bzip2形式でのバックアップ．
何の形式で圧縮したかわからなくならないよう，拡張子をbz2にすること
#+BEGIN_SRC bash
# tar jcvf ./bk.tar.bz2 ./
ls -l  bk.tar.bz2
#+END_SRC
...
#+RESULTS:
: -rw-r--r--  1 ahayashi  staff  367291 Jan  2 15:42 bk.tar.bz2

*** 2つのディレクトリを同期する
バックアップと同期は厳密には違う．

**** 単純なコピー rsync
rsync -az 元dir/ 同期する新dir/
- a: ファイルの属性をなるべく残す．
dir の最後にはスラッシュを付けること．

**** 2回目以降の時短効果
差分だけ同期するので2回目以降は少し速い．
システムがファイルだけでできていれば，引っ越しも基本的にrsyncだけで済む．
同期は手動でやるのがいい．
なぜなら，自動同期システムでは，ファイルが壊れたときにも自動的に同期されてしまう可能性がある．
同期対象外しは--exclude．

**** 完全に同期する
こうやると，バックアップとは言えなくなる．
ハードディスクの内容を別の場所に完全同期している状態で，
ディスクがアンマウントされると，同期先も空になってしまう．

**** ほかのマシンと同期する
今は ssh 接続が基本なので，ssh を明示する必要はない．
*** 最新ファイルを古いファイルで上書きしないように同期を行う（雑な方法）
rsync -u
-u オプションをつけると，タイムスタンプを比較して，新しい方を残すようにしてくれる．
ただし，両方のファイルを同期前に編集していたら，当然片方（より新しい方）しか残らないので注意
** 圧縮されたファイルの操作
*** FUTURE gzipされたファイルを圧縮されたまま使う
圧縮はgzip．解凍はgunzip．
ただしいちいち解凍するのは面倒．
zcat (Macではgzcat!)すると，解凍せずにそのまま表示できる．

zgrep について以降はわからないので飛ばした
*** FUTURE アーカイブの中から特定のファイルだけを抜き出す
わからないので飛ばした
** ファイルの整理
*** 重複したファイルを消す md5sum, cmp, yarr

名前が重複していたとしても，中身が同じとは限らない．
cmpを使うことを思いつくが，ファイル数が莫大な場合は，いつまでたっても終わらない．
ファイルの中身から「MD5値」を計算するmd5sumを使って，第1フィールドの16進数を取得する．

これに，yarr　（横アレー） というTukubaiコマンドをかけ，同じキーを持つレコードを横並びで表示させる．

これで容疑者を割り出し，これに対してcmpする．

*** ディレクトリの古いファイルを整理する ls -lTr, date -d
Macの場合は--full-time がサポートサれていないので-Tオプションにする．

date -d '10 days ago'がエラー．
-d オプションの動きが違う？

p.134 dir内を掃除するスクリプトが書いてある．
shebang に -e オプションをつけると，0以外の終了ステータスでプログラムを止めることができる．
これを自動化するのは後ろの章．
今回は手で動かす．
xargs -I【目印】は，【目印】のところに標準入力の文字列を渡すことができる．
#+BEGIN_SRC bash

# ls -lTra | awk -v "d=$(date -d '10 days ago') +%Y-%m-%d" 'd>=$6' | tail -n 10
#+END_SRC
...
*** 複数のテキストファイルを圧縮しないで1つにまとめる
テキストファイルを，dir構造もテキストで表しながら，1つのファイルにまとめる．
逆もできる．
** ファイルの暗号化
- 公開鍵暗号方式: ちゃんとした方法
- パスワード方式: 弱い
*** ファイルを公開鍵暗号方式で暗号化する
**** 鍵の作成 *それぞれのコマンドで何をやっているのだろう？*
...
すると，pubkey.pem （公開鍵）と seckey.pem （秘密鍵）ができている．
#+BEGIN_SRC bash
ls *pem
#+END_SRC
...
**** 秘密のファイルを作る
#+BEGIN_SRC bash
echo 'ここだけの話，部長の鼻毛，付け鼻毛らしいよ．' > himitsu
cat himitsu
#+END_SRC

#+RESULTS:
: ここだけの話，部長の鼻毛，付け鼻毛らしいよ．
**** 暗号化してみる
#+BEGIN_SRC bash
openssl rsautl -pubin -inkey pubkey.pem -in himitsu -encrypt -out himitsu.secret
#+END_SRC
...
**** 内緒のファイルを作る
#+BEGIN_SRC bash
echo '内緒なのですが，隣の奥さん，ジャンボタニシなんですって．' > naishyo.txt
cat naishyo.txt
#+END_SRC

#+RESULTS:
: 内緒なのですが，隣の奥さん，ジャンボタニシなんですって．
**** パスワード付きで圧縮する
#+BEGIN_SRC bash
zip -P 1234 naishyo.zip naishyo.txt
#+END_SRC

#+RESULTS:
:   adding: naishyo.txt (deflated 5%)
**** 読もうとしてみる
そなままだともちろん文字化けして読めない
#+BEGIN_SRC bash
#cat naishyo.zip
#+END_SRC
...
破れてしまう．恐ろしい！！
#+BEGIN_SRC bash
seq -w 0 9999 | while read n ; do unzip -P $n -pq naishyo.zip ; done > log
cat log
#+END_SRC
...
2秒で破れてしまう
#+BEGIN_SRC bash
seq -w 0 9999 | while read n ; do unzip -P $n -pq naishyo.zip && exit 0; done 2> /dev/null
#+END_SRC
...
*** プログラムを定期的に自動実行する
cron(8)を使う．
Ubuntu では最初からcronが起動しているらしいが，Macでは起動していないみたいだが，一応入っているようだ．
#+BEGIN_SRC bash
ps cax | grep cron
#+END_SRC
...
例ではcrontab -e で，親切な解説付きの設定ファイルが出ることになっていたが，
自分の環境ではまっさらなファイルが開いた．

時刻の引数は左から，分/時/日/月/曜日となっている．

crantabの環境は普段の端末環境同じではないので，コマンドが通らないことがある．
落ち着いて，パスを追加すること．

*** ファイルが変更されたらアクションを起こす fswatch   :難しいので流し読み:
inotify(7)を使うとあるが，Mac にはないのでfswatchを使う．
プログラムを書いて保存したら，自動でコンパイルする，などに応用できる．


make と組み合わせることもできる．p.151

ただし，make 中の更新には反応できない．
makeには時間がかかる場合，その更新は検知できない．

& の使い方については [[*%E3%83%90%E3%83%83%E3%82%AF%E3%82%B0%E3%83%A9%E3%82%A6%E3%83%B3%E3%83%89%E5%87%A6%E7%90%86%E3%81%A7%E4%B8%A6%E5%88%97%E5%8C%96%E3%81%99%E3%82%8B][バックグラウンド処理で並列化する]] を参照．
*** service(1)で自作のプログラムを起動/停止する
automake を service を通じて実行してみる
service はルートに移動してからプログラムを実行するので，
automake のファイルのディレクトリは絶対パスで書くこと．
tips: cd と make を && でつなぎ，括弧に入れると，make が終わった後には cd がなかったことになる．
これを使わないときと比べて，どんないいことがあるんだろう？
*** OS起動時に自作のプログラムを走らせる  :難しいので流し読み:
automake を OS 起動時に走らせる．
シンボリックリンクを置くとできる．
update-rc.d を実行するとシンボリックリンクが一気に生成される．
以下略．
今の所 [2019-01-14 Mon] 理解できないし，使う予定もなさそうなので飛ばす．
**** プロセスグループID/ セッションIDを確認/ 利用する
ID の種類
- プロセスID: 全てのコマンドは全て違うID
- プロセスグループID: パイプでつながったコマンドが同じIDになる
- セッションID: 同じ端末で実行されたプロセスは全て同じIDになる
#+BEGIN_SRC sh
echo $$
#+END_SRC
...
#+begin_example
  PID  PGID ARGS
    1     1 /sbin/launchd
   59    59 /usr/sbin/syslogd
   60    60 /usr/libexec/UserEventAgent (System)
   64    64 /System/Library/PrivateFrameworks/Uninstall.framework/Resources/uninstalld
   65    65 /usr/libexec/kextd
   66    66 /System/Library/Frameworks/CoreServices.framework/Versions/A/Frameworks/FSEvents.framework/Versions/A/Support/fseventsd
   68    68 /System/Library/PrivateFrameworks/MediaRemote.framework/Support/mediaremoted
   71    71 /System/Library/CoreServices/appleeventsd --server
   72    72 /usr/sbin/systemstats --daemon
#+end_example
...
#+RESULTS:
: -rw-r-----@ 1 root  admin  61845 Jan 15 21:15 /var/log/system.log
: -rw-r-----  1 root  admin  21178 Jan 15 01:48 /var/log/system.log.0.gz
: -rw-r-----  1 root  admin  15594 Jan 14 00:50 /var/log/system.log.1.gz
: -rw-r-----  1 root  admin  23228 Jan 13 01:18 /var/log/system.log.2.gz
: -rw-r-----  1 root  admin  17865 Jan 12 01:39 /var/log/system.log.3.gz
: -rw-r-----  1 root  admin  14250 Jan 11 01:45 /var/log/system.log.4.gz
: -rw-r-----  1 root  admin  28122 Jan 10 01:25 /var/log/system.log.5.gz
: -rw-r-----  1 root  admin  10815 Jan  9 01:50 /var/log/system.log.6.gz

*** 検索する
cat を grep や awk に渡して解析する．
ただ，日付が DD dd 形式なので解析しづらい．
*** 日付を正規化する
**** bash スクリプトを書く場合
#!/bin/bash
...
なるほど．
**** 日付の正規化
tsukubai の dayslash を使う
*** Apache のログ（不定長データ）をスペース区切りに整形する
正規表現大活躍．
*** データをキーバリュー形式に整形する
cat を juni（行番号を表示）とmaezero （ゼロ埋め）に渡して，キーを自作する
** プロセスに関する注意点
*** 多数のプロセスから1つのログファイルに書き込む
自分のプログラムにログを吐かせるtips
ログファイルは一つでいい．
よっぽど下手なプログラムを書かない限り，ログファイルがビジーになって壊れることはない（わざと壊した例はp. 171）．
*** 制御構文が動作するプロセスについて
exit 0 の挙動がおかしいとき，標準入力があるかどうか（ssh接続中など？わからない）を確認せよ．
...
#+BEGIN_SRC bash
cd example
cat xl/worksheets/sheet1.xml | # 改行されていない！！
 grep -o 'r="[A-Z]*[0-9]*"><v>[^<]*' | # <c r="A3"><v>3</v></c> みたいなタグを頼りに，改行する．
 tr '"<>' ' '| awk '{print $2,$4}' # セル位置と値を取り出す
#+END_SRC
...
cd example
cat xl/worksheets/sheet1.xml | # 改行されていない！
 hxselect c |# データは取り出せたがまだ改行がない!
 sed 's;</c>;&\\n;g' # babel 環境では表示されないけどできた！
#+END_SRC
...
#+BEGIN_SRC bash
cd example
cat xl/worksheets/sheet1.xml | hxselect c | sed 's;</c>;&\\n;g'
#+END_SRC

#+RESULTS:
: <c r="A1"><v>1</v></c>\n<c r="C1" t="s"><v>0</v></c>\n<c r="A2"><v>2</v></c>\n<c r="C2" t="s"><v>1</v></c>\n<c r="A3"><v>3</v></c>\n<c r="C3" t="s"><v>2</v></c>\n<c r="A4"><v>-4.2300000000000004</v></c>\n
**** 文字はsharedStrings にある
この番号とsheet1.xml の番号が対応しているというまどろっこしい方法．
#+BEGIN_SRC bash
cd example
...
: <sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="3" uniqueCount="3"><si><t>方</t></si><si><t>眼</t></si><si><t>紙</t></si></sst>
**** 各セルの文字列を取り出すにはどうするか？
- sharedStrings.xml の si 要素に番号をつけてマスタを作る
- sheet1.xml の文字列のセルと join する
これをやるために，Tsukubai の cjoin1 が便利．
**** 文字列のマスタファイルを作る
#+BEGIN_SRC bash :results vervatim
cd example
#cat xl/sharedStrings.xml | hxselect si | sed 's;</si>;&\\n;g' | sed 's/<si><t>//' | sed 's/<.*//' | awk '{print NR-1,$1}' > master
cat master
#+END_SRC
...
#+BEGIN_SRC bash
cd example
#cat xl/worksheets/sheet1.xml | hxselect c | sed 's;</c>;\n;g' | grep 't="s"' | sed 's;<c r="\(.*\)" t="s"><v>\(.*\)</v>;\1 \2;' > tmp
cat tmp
#+END_SRC
...
#+BEGIN_SRC bash
cd example
cat tmp | cjoin1 key=2 master ~ | delf 2 > strings
cat strings
cat tmp
#+END_SRC
